...
const Contact = () => {
	...
	const sendMessage = data => {
		try {
			email.sendForm(VITE_SERVICE_ID, VITE_TEMPLATE_ID, form.current, VITE_PUBLIC_KEY);
			reset();
		...
		<div className='mt-20 px-10 py-4 flex'>
			...
			<form className="w-[40%] flex flex-col bg-light-300 p-6 rounded-l-lg" ref={form} onSubmit={handleSubmit(sendMessage)}>
				...
				<div className={`flex flex-col ${errors.name ? "mb-0" : "mb-2" }`}>
					...
					<input 
						...
						{...register("name", { 
							...
							maxLength: { value: 40, message: "El nombre no puede tener más de 40 caracteres" }
						})} 
					/>
					{errors.name && <span className="text-red-600 dark:text-red-600">{errors.name.message}</span>}
				</div>
				<div className={`flex flex-col ${errors.email ? "mb-0" : "mb-2" }`}>
					...
					<input 
						...
						{...register("email", { 
							...
							pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, message: "Debe ser un email válido" }
						})}
					/>
				...
				<div className={`flex flex-col ${errors.message ? "mb-0" : "mb-2" }`}>
					...
					></textarea>
					{errors.message && <span className="text-red-600 dark:text-red-600">{errors.message.message}</span>}
				</div>
