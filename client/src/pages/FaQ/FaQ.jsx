...
const FaQ = () => {
  ...
  const questions = [
    ...
    },
    {
      ...
      answer: "Cuando usted se registró en la página nos proporcionó una dirección de e-mail a la que enviamos todas nuestras comunicaciones incluyendo las de confirmación de compras. Si usted no encuentra el mail de confirmación, por favor tenga en cuenta los siguientes puntos: - Asegúrese de estar revisando la casilla de mail correspondiente. - Revise si el mail de confirmación se encuentra en las carpetas de SPAM o correo no deseado, si ese es el caso, por favor marque el remitente info@cinego.com como remitente confiable para que no se repita el inconveniente. - Algunas casillas de e-mail laborales o profesionales rechazan los mensajes provenientes de remitentes no autorizados, le recomendamos que no se registre en la página con este tipo de direcciones de e-mail. - Si a pesar de haber seguido estas indicaciones no encuentra la información, envíenos su consulta."
    },
    {
      id: 5,
      question: "¿Puedo cancelar mi pedido de devolución?",
      answer: "Lamentablemente no podemos cancelar un pedido de devolución ya inciado. Usted puede hacer una nueva compra pero no podemos garantizar que las localidades que habia solicitado devolver se encuentren nuevamente disponibles.",
    },
    ...
    <div className="space-y-6">
      {questions.map((q) => (
        <div key={q.id} className="border border-gray-400 p-5 rounded-lg">
          <button
            className="flex justify-between w-full hover:text-blue-800"
            onClick={() => toggleQuestion(q.id)}
          >
            <span className="text-x1 text-justify font-semibold hover:text-blue-800">{q.question}</span>
            <svg
              className={`h-6 w-6 ${
